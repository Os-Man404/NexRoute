...
// Junction (J1) turn settings - confirmed: X=Left, Y=Right.
// Return-to-base direction at J1 is a placeholder - adjust after testing on the real track.
char J1_TURN_TO_X = 'L';
char J1_TURN_TO_Y = 'R';
char J1_TURN_RETURNING = 'R';   // <-- TODO: confirm real direction, currently a guess

bool isTurningAtJunction = false;
unsigned long turnStartTime = 0;
const unsigned long JUNCTION_TURN_MS = 500;   // open-loop pivot duration, tune on real track
...
<div class="panel" id="p3">
  ...
  <button class="go" onclick="send('/call')">CALL ROBOT (Go to X)</button>
  <div class="sp"></div>
...
setInterval(()=>{fetch('/status').then(r=>r.json()).then(j=>{
  ...
  rstate.innerText = j.robotState + (j.auto ? ' (AUTO)' : ' (idle)');
...
void handleCall() {
  ...
  Serial.println("[APP] Call received. Robot heading to X.");
  server.send(200, "text/plain", "ok");
}

void handleItemLoaded() {
  if (robotState == STATE_AT_X) {
    robotState = STATE_GOING_TO_Y;
    integral = 0; lastError = 0;
    Serial.println("[APP] Item Loaded confirmed. Robot heading to Y.");
    server.send(200, "text/plain", "ok");
  } else {
...
void handleReceived() {
  if (robotState == STATE_AT_Y) {
    robotState = STATE_RETURNING_TO_BASE;
    integral = 0; lastError = 0;
    Serial.println("[APP] Received confirmed. Robot returning to base.");
    server.send(200, "text/plain", "ok");
  } else {
